const e = require('express');
var express = require('express');

var router = express.Router();

var pool = require('./db');

// router.get('/hello',(req,res)=>{
//     pool.query('SELECT * FROM posts',(q_err,q_res) =>{
//          res.json(q_res.rows)
//         // console.log(q_res.rows)
//     })
// })

// posts route section starts
router.get('/api/get/allposts',(req,res,next) => {
    pool.query('SELECT * FROM posts ORDER BY date_created DESC',(q_err,q_res) =>{
         res.json(q_res.rows)
    })
})

// router.post('/api/post/posttodb',(req,res,next)=>{
//     const values = [req.body.title,req.body.body,req.body.uid,req.body.username]
//     console.log(values)
//     pool.query(`INSERT INTO posts(title,body,user_id,author,date_created) VALUES($1,$2,$3,$4,NOW())`,values,(q_err,q_res)=>{
//         if(q_err) return next(q_err)
//         res.json(q_res.rows);//return status 200 if everything is okay
//     })

// })

router.post('/api/post/posttodb',(req,res,next)=>{

    const body_vector = String(req.body.body)
    const title_vector = String(req.body.title)
    const username_vector = String(req.body.username)

    const search_vector = [title_vector,body_vector,username_vector]

    const values = [req.body.title , req.body.body , search_vector , req.body.uid , req.body.username]
    console.log(values)
    pool.query(`INSERT INTO posts(title,body,search_vector,user_id,author,date_created) 
                VALUES($1,$2,to_tsvector($3),$4,$5,NOW())`,values,(q_err,q_res)=>{
        if(q_err) return next(q_err)
        res.json(q_res.rows);//return status 200 if everything is okay
    })

})

//search get req
router.get('/api/get/userposts/:id/:search',(req,res,next) => {
    const user_id = String(req.params.id)
    const search_query = String(req.params.search)
    const values = [user_id,search_query]
   

        pool.query(`SELECT * FROM posts WHERE user_id=$1 AND
        search_vector @@ to_tsquery($2)`,values,(q_err,q_res)=>{
            if(q_res!=undefined){

                res.json(q_res.rows)
                console.log(q_err)
            }
        })
    
})



router.put('/api/put/post',(req,res,next)=>{

    const body_vector = String(req.body.body)
    const title_vector = String(req.body.title)
    const username_vector = String(req.body.username)

    const search_vector = [title_vector,body_vector,username_vector]

    const values = [req.body.title , req.body.body , req.body.uid , req.body.pid , req.body.username , search_vector]
    pool.query(`UPDATE posts SET title=$1, body=$2,search_vector=to_tsvector($6), user_id=$3, author=$5, date_created=NOW()
    WHERE pid=$4`,values,(q_err,q_res)=>{
        res.json(q_res.rows)
    })
})

router.delete('/api/delete/postcomments',(req,res,next)=>{
    const post_id = req.body.post_id;
    pool.query(`DELETE FROM comments WHERE post_id = $1`,[post_id],(q_err,q_res)=>{
        res.json(q_res.rows)
        console.log(q_err)
    })
})

router.delete('/api/delete/post',(req,res,next)=>{
    const post_id = req.body.post_id;
    pool.query(`DELETE FROM posts WHERE pid=$1`,[post_id],(q_err,q_res)=>{
        res.json(q_res.rows)
        console.log(q_err)
    })
})

// posts route section ends

//comment route section starts

router.post('/api/posts/commenttodb',(req,res,next)=>{
    const values = [req.body.comment,req.body.user_id,req.body.username,req.body.post_id]
    pool.query(`INSERT INTO comments(comment,user_id,author,post_id,date_created) VALUES($1,$2,$3,$4,NOW())`,values,(q_err,q_res)=>{
        //if(q_err) return next(q_err)
        res.json(q_res.rows);//return status 200 if everything is okay
        console.log(q_err)
    })

})

router.put('/api/put/commenttodb',(req,res,next)=>{
    const values = [req.body.comment,req.body.user_id,req.body.post_id,req.body.username,req.body.cid]
    pool.query(`UPDATE comments SET comment=$1, user_id=$2, post_id=$3, author=$4, date_created=NOW()
    WHERE cid=$5`,values,(q_err,q_res)=>{
        res.json(q_res.rows)
    })
})




router.delete('/api/delete/comment',(req,res,next)=>{
    const cid = req.body.cid;
    pool.query(`DELETE FROM comments WHERE cid=$1`,[cid],(q_err,q_res)=>{
        res.json(q_res.rows)
        console.log(q_err)
    })
})


router.get('/api/get/allpostcomments/:id',(req,res,next)=>{

    const post_id = String(req.params.id)
    pool.query(`SELECT * FROM comments WHERE post_id=$1`,[post_id],(q_err,q_res)=>{
        res.json(q_res.rows)
        console.log(q_err)
    })
})


//comment route section ends


//userprofile route section starts
router.post('/api/post/userprofiletodb',(req,res,next)=>{
    const values = [req.body.profile.nickname,req.body.profile.email,req.body.profile.email_verified]
    pool.query(`INSERT INTO users(username,email,email_verified,date_created)
    VALUES($1,$2,$3,MOW())
    ON CONFLICT DO NOTHING`,values,(q_err,q_res)=>{
        res.json(q_res.rows)
        console.log(q_err)
    })
})



router.get('/api/get/userprofiletodb',(req,res,next)=>{
    const email = String(req.body.email);

    pool.query(`SELECT * FROM users WHERE email=$1`,[email],(q_err,q_res)=>{
        res.json(q_res.rows)
        console.log(q_err)
    })
})
/// check 
router.post('/api/post/userprofilecheck',(req,res,next)=>{
    const email = [req.body.email]
    // console.log(email)
    pool.query(`SELECT * FROM users WHERE email=$1`,email,(q_err,q_res)=>{
        res.json(q_res.rows)
        console.log(q_err)
    })
})
///
router.get('/api/get/userposts/:id',(req,res,next)=>{
    const user_id = String(req.params.id)
    pool.query(`SELECT * FROM posts WHERE user_id=$1`,[user_id],(q_err,q_res)=>{
        res.json(q_res.rows)
        console.log(q_err)
    })
})

// userprofile route section ends
module.exports = router;